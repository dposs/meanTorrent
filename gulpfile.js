...
gulp.task("env:test", async function () {
...
gulp.task("copyLocalEnvConfig", function (done) {
  ...
  if (!fs.existsSync("config/env/" + renameTo)) {
    return gulp.src("config/env/local.example.js")
...
gulp.task("makeUploadsDir", async function () {
  fs.mkdir("modules/users/client/img/profile/uploads", function (err) {
...
gulp.task("mocha:coverage", function () { // ["pre-test", "mocha"]
  var testSuites = changedTestFiles.length ? changedTestFiles : testAssets.tests.server;
...
gulp.task("protractor", function () { // ["webdriver_update"]
